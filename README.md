...
### 📜 CNB 构建日志审阅（与 GitHub Actions 类似的分步结构）

选择 CNB 执行器时，CNB 流水线（`.cnb.yml`）拆分为 3 个**命名步骤**，构建日志按步骤分组，方便审阅：

| 步骤 | 内容 |
|---|---|
| **1/3 准备环境** | Node/npm 版本、系统工具安装、undici 依赖、任务参数 |
| **2/3 配置 Clash 代理（可选）** | mihomo 安装/版本、订阅下载与格式识别（yaml/base64/URI）、节点数与配置行数、启动与监听验证（`CLASH_PROXY`） |
| **3/3 执行构建器 Agent 任务** | runner 内部再分 4 步：拉取任务（DNS+重试）→ Agent 生成循环（每轮字数/工具结果）→ 提取代码 → 部署与冒烟测试，全部带时间戳 |

每个步骤失败时日志会打印明确原因（mihomo 日志尾部、订阅转换日志、runner 错误码），便于定位。
...
- **2026-08-14**：v2.5.0 前端增量渲染 + 冒烟/压缩异步化（P0 性能优化）
  ...
  - **上下文压缩延后**：`maybeCompact` 从对话开始前同步执行移到对话结束后后台执行，用户发消息不再被压缩阻塞（此前长对话首条消息需额外等 3-5 秒）

- **2026-08-11**：v2.4.0 CNB 构建日志分步结构化（参考 GitHub Actions）
  - `.cnb.yml` 拆分为 3 个命名步骤：准备环境 / 配置 Clash 代理 / 执行 Agent 任务，日志按步骤分组
  - 每步输出：版本信息、命令执行详情、订阅格式与节点数、代理监听验证、失败原因（mihomo/转换日志）
  - runner 日志加时间戳与 4 步分节（拉取任务 / 生成循环 / 提取代码 / 部署冒烟）
